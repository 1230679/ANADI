...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\linewidth]{../fig_confusion_matrix.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{../fig_feat_imp_clf.png}
...
Este trabalho completou a transi\c{c}\~ao metodol\'ogica iniciada no TP1
-- de uma an\'alise de regress\~ao linear agregada ao n\'ivel do concelho
($R^2_{adj} = 6.4\%$) para uma modelação biv\'ia ao n\'ivel do PTD
individual ($\approx69\,000$ registos) -- abordando simultaneamente um
problema de \textbf{regress\~ao} (prever a folga de pot\^encia dispon\'ivel
\texttt{PFolga\_PTD}) e um problema de \textbf{classifica\c{c}\~ao} (prever
o n\'ivel de ocupa\c{c}\~ao da rede \texttt{utilizRede}), com o objetivo
final de suportar decis\~oes de integra\c{c}\~ao de infraestrutura de
carregamento de ve\'iculos el\'etricos.

No problema de regress\~ao, a Rede Neuronal Shallow emergiu como o modelo
mais preciso (MAE~$= 32.05 \pm 0.81$~kVA, RMSE~$= 52.13$~kVA em
10-\textit{fold} CV), confirmada como estatisticamente superior ao SVM
linear (MAE~$= 33.22$, $p < 0.05$, \textit{t-test} pareado). A Regress\~ao
Linear Simples, baseada unicamente em \texttt{Cap\_PTD\_kVA} ($r = 0.92$),
estabeleceu a \textit{baseline} com MAE~$\approx 51$~kVA, evidenciando que
a capacidade instalada do transformador \'e o fator estrutural dominante
da folga de rede. A \'Arvore de Regress\~ao (MAE~$= 34.18$, $d = 7$) revelou
robustez face aos \textit{outliers} de PTDs urbanos de grande porte --
que violam os pressupostos de homoscedasticidade da regress\~ao linear --
sem recorrer a transforma\c{c}\~oes do \textit{target}. O MAE residual de
$\approx$32~kVA na melhor configura\c{c}\~ao reflete o teto informativo
atual das \textit{features} dispon\'iveis: a forte assimetria positiva da
distribui\c{c}\~ao de \texttt{PFolga\_PTD} e a aus\^encia de dados de
gera\c{c}\~ao distribu\'ida limitam a precis\~ao absoluta independentemente
do algoritmo adotado.

No problema de classifica\c{c}\~ao, a \textbf{\'Arvore de Decis\~ao com
profundidade~7} constitui o modelo mais adequado ao contexto operacional,
apesar de o SVM~(RBF) registar \textit{Accuracy} marginalmente superior
($0.6903$ vs.\ $0.6884$). O teste \textit{t} pareado ($t = 0.698$,
$p = 0.503$, $\alpha = 0.05$) confirma que esta vantagem n\~ao \'e
estat\'isticamente significativa. Os crit\'erios secund\'arios pendem
inequivocamente para a \'Arvore: melhor F1 ponderado ($0.6773$ vs.\
$0.6668$), melhor \textit{Recall} na classe \textit{m\'edio} ($0.39$ vs.\
$0.30$) e interpretabilidade direta atrav\'es das regras de decis\~ao e
das import\^ancias Gini -- atributos indispens\'aveis numa aplica\c{c}\~ao
de planeamento de infraestrutura regulada.

O \textbf{calcanhar de Aquiles} de todos os classificadores \'e a classe
\textit{m\'edio}: nenhum modelo ultrapassa \textit{Recall}~$= 0.39$, e a
matriz de confus\~ao da \'Arvore revela que 1\,473 dos 3\,732~PTDs
\textit{m\'edio} s\~ao erroneamente sinalizados como \textit{baixo}. Numa
aplica\c{c}\~ao operacional, estes falsos positivos traduzem-se em PTDs em
``zona de aten\c{c}\~ao'' incorretamente identificados como aptos para
receber carregadores de VE -- o risco operacional mais relevante do
classificador. A causa-raiz \'e estrutural: \texttt{Util\_Decimal} assume
apenas 6 valores distintos, tornando as fronteiras de decis\~ao
artificialmente difusas e produzindo o desequil\'ibrio intr\'inseco de
$\approx51/25/24$\% que penaliza sistematicamente a classe intermedi\'aria.

A an\'alise de import\^ancia de \textit{features} confirma, de forma
taxativa, que o verdadeiro determinante da ocupa\c{c}\~ao da rede \'e a
\textbf{capacidade por cliente} (\texttt{Cap\_per\_Cliente}, 70\% da
import\^ancia Gini), complementada pela pot\^encia contratada por cliente
(\texttt{PContratada\_per\_Cliente}, $\approx$19\%). Em contraste, todas
as vari\'aveis de Ilumina\c{c}\~ao P\'ublica apresentam import\^ancia
Gini pr\'atica nula: o ganho mediano da transi\c{c}\~ao LED por PTD
($\approx 0.32$~kVA) \'e negligenci\'avel face \`a folga mediana de
$\approx 77$~kVA, alterando o estado de viabilidade de apenas 1~PTD em
todo o \textit{dataset}. Esta conclus\~ao, que aprofunda e corrobora o
resultado do TP1, imp\~oe uma reorienta\c{c}\~ao estrat\'egica: a
moderniza\c{c}\~ao LED \'e politicamente relevante no plano da
sustentabilidade energ\'etica global, mas insuficiente, por si s\'o,
para criar folga de rede mensur\'avel ao n\'ivel do PTD individual que
suporte a carga de um carregador t\'ipico de 22~kW.

Os modelos aqui desenvolvidos constituem, em conjunto, um sistema de
apoio \`a decis\~ao tecnicamente fundamentado para Operadores de Rede de
Distribui\c{c}\~ao como a e-REDES: a regress\~ao quantifica a folga
dispon\'ivel em cada PTD, e a classifica\c{c}\~ao prioriza os postos
candidatos \`a instala\c{c}\~ao de infraestrutura de carregamento. A sua
maturidade operacional plena depende, contudo, de dois passos futuros
priorit\'arios -- a aplica\c{c}\~ao de SMOTE ao conjunto de treino de
cada \textit{fold} para corrigir o \textit{Recall} catastr\'ofico na
classe \textit{m\'edio}, e a recupera\c{c}\~ao dos valores cont\'inuos
de \texttt{Util\_Decimal} em colabora\c{c}\~ao com a e-REDES -- sem os
quais qualquer classficador enfrenta um limite estrutural intranspon\'ivel
que compromete a fiabilidade das recomenda\c{c}\~oes para o planeamento
da mobilidade el\'etrica em Portugal.
